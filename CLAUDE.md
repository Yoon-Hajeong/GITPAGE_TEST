# YOONHAJEONG — Personal Wiki + Two-Site Publishing System

Obsidian 기반 개인 위키. **단일 마크다운 source → 두 개 GitHub Pages 사이트로 빌드**.
...
│   │                       Editorial design (Cormorant + Pinyon Script, 크림색 종이)
│   │
...
|--------|------|--------------------|-----|
| **public** (사이트 A) | 구독자 모집용 / 공개 마케팅 | frontmatter `publish: true` 인 파일만 | `yoonhajeong.github.io/wiki/` |
| **personal** (사이트 B) | 본인 공개 블로그 / 전체 노출 | WIKI 전체 | `yoonhajeong.github.io/wiki/notes/` |
...
### 동기화/배포 트리거
- "동기화" / "sync" / "동기화해" → `scripts/sync-wiki.mjs` 실행
...
cd sites/public && npm install && npm run dev   # http://localhost:4321

# 사이트 B (personal) 로컬 미리보기
cd sites/personal && npm install && npx quartz build --serve   # http://localhost:8080
```
